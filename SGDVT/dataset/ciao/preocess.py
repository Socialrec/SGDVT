...
d = sio.loadmat(f'./rating.mat')
prime = []
for val in d['rating']:
    user, item,  rating = val[0], val[1], val[4]
    prime.append([user, item, rating])
df = pd.DataFrame(prime, columns=['user', 'item',  'rating'])
df['rating']=1
#print(df)
del prime, d, user, item, rating
gc.collect()


d = sio.loadmat(f'./trustnetwork.mat')
prime = []
for val in d['trustnetwork']:
    user, friend = val[0], val[1]
    prime.append([user, friend])
Socialnet = pd.DataFrame(prime, columns=['user', 'friend'])
#print(Socialnet)
...
del_threshold = [4,4,4,4,4,4,4,4,4]
